...
export const handleImageUpload = async (
  ...
  try {
    ...
    // Convert to base64
    const base64String = await convertFileToBase64(file, abortSignal);
...
export const createPlaceholderImage = (
  ...
  const svg = `
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      <rect width="${width}" height="${height}" fill="#f4f4f4" stroke="#ddd" stroke-width="1"/>
      <text x="50%" y="50%" font-size="14" fill="#999" text-anchor="middle" dy="0.3em">
        ${width}x${height}
      </text>
    </svg>
  `;
